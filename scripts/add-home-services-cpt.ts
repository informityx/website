/**
 * Creates or updates the `our-services` Custom Type, a `cards` section on it, and a
 * `customPostType` section on the home page (Services teaser). Copy lives in
 * scripts/services-cpt-content.ts (docs/inforMityx.ai.pdf Services themes).
 *
 * Usage: npx tsx scripts/add-home-services-cpt.ts
 * API path: CMS_ADMIN_PASSWORD + NEXT_PUBLIC_APP_URL (default http://localhost:9000)
 * Prisma path: omit CMS_ADMIN_PASSWORD (direct DB; same shapes as API after parse).
 */
...
async function tryApi(baseUrl: string, email: string, password: string): Promise<boolean> {
  ...
  let ct = allTypes.find((t) => t.slug === SERVICES_CPT_SLUG)
  ...
  const cardsContent = buildServicesCardsSectionContent()
  ...
  if (existingCards) {
    const put = await authFetch(`${baseUrl}/api/sections/${existingCards.id}`, {
      ...
      body: JSON.stringify({
        ...
        order: SERVICES_CARDS_SECTION_ORDER,
        isVisible: true,
      }),
    })
    if (!put.ok) {
  ...
  let order = SERVICES_HOME_SECTION_ORDER
  const taken = homeSections.some((s) => s.order === order)
  if (taken) {
    const maxOrder = homeSections.reduce((m, s) => Math.max(m, s.order), 0)
    order = maxOrder + 1
    console.log(
      `Home order ${SERVICES_HOME_SECTION_ORDER} taken; placing customPostType at order ${order}. Reorder in admin if needed.`
    )
  }
  ...
  console.log("  itemsToShow:", SERVICES_ITEMS_TO_SHOW, "customTypeId:", ct.id)
  return true
}
...
async function viaPrisma(): Promise<void> {
  ...
  let ct = await prisma.customType.findUnique({ where: { slug: SERVICES_CPT_SLUG } })
  if (!ct) {
  ...
  if (homeSections.some((s) => s.order === order)) {
    ...
    order = maxOrder + 1
    console.log(
      `Prisma: order ${SERVICES_HOME_SECTION_ORDER} taken; using order ${order}. Reorder in admin if needed.`
    )
  }
